...
# --- CONFIGURATION v5.6 "APEX-GUARDIAN" ---
# Version finale stabilisée : Agressivité 4.9.8 + Rigueur 5.3 + Correctifs Audit 5.5
...
GOUVERNANCE = {
    ...
    "MAX_TOTAL_DRAWDOWN_PCT": 0.10,     # Rétabli : Protection long-terme
...
SYSTEM_STATE = {
    "status": "starting",
    "equity": 0.0,
    "engine_version": "5.6-Apex-Guardian",
    "trades_today": 0,
...
class TitanEngine:
    ...
    def _init_db(self):
        """Initialisation de la structure de données locale."""
        with sqlite3.connect(DB_PATH) as conn:
            ...
            conn.execute("""CREATE TABLE IF NOT EXISTS trades (
                ...
                sector TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)""")
    ...
    def _load_anchor(self):
        """Charge le capital de départ pour le calcul du Drawdown Total."""
        with sqlite3.connect(DB_PATH) as conn:
    ...
    def get_sector_with_cache(self, symbol):
        """Centralisation du cache sectoriel pour éviter les incohérences (Audit 5.5)."""
        with sqlite3.connect(DB_PATH) as conn:
            res = conn.execute("SELECT sector FROM sector_cache WHERE symbol=?", (symbol,)).fetchone()
            if res: return res[0]
        
        try:
            # Si absent, on interroge yf et on met en cache
    ...
    async def reconcile_trades(self):
        """Réconciliation Broker -> DB avec tri chronologique (Audit 5.5)."""
        try:
            ...
            with sqlite3.connect(DB_PATH) as conn:
                ...
                for c_id, sym, entry, qty, ts in cursor.fetchall():
                    ...
                    if (datetime.now() - entry_dt).days >= GOUVERNANCE["MAX_HOLDING_DAYS"]:
                        ...
                        logging.info(f"⏳ Time-Exit activé : {sym}")
                        continue

                    # 2. PnL Sync avec tri chronologique des ordres clos
                    if sym not in positions:
                        orders = self.alpaca.list_orders(status='closed', limit=10, symbols=[sym])
                        if orders:
                            # Tri par date de remplissage pour obtenir le dernier ordre de vente réel
                            ...
                            logging.info(f"🔄 Réconciliation terminée pour {sym} : PnL enregistré.")
                conn.commit()
        except Exception as e: logging.error(f"Erreur Réconciliation : {e}")

    def sync_forge(self):
        """Triage adaptatif basé sur la performance réelle."""
        with sqlite3.connect(DB_PATH) as conn:
            cursor = conn.cursor()
    ...
    async def fetch_signals(self, session):
        """Scan rapide Alpha Vantage (Esprit 4.9.8)."""
            ...
            async with session.get(url) as resp:
                ...
                for _, row in today_earnings.iterrows():
                    ...
                    try:
                        ...
                        # Utilisation du cache sectoriel centralisé
                        sector = self.get_sector_with_cache(sym)
                        candidates.append({"symbol": sym, "price": trade.price, "sector": sector})
                    except: continue
        except: pass
        return candidates

    async def get_ai_score(self, session, c):
        """IA non-bloquante (Esprit 4.9.8)."""
        if not OR_KEY: return 82.0, 12.0
        try:
            prompt = f"PEAD Analysis {c['symbol']}. Price: {c['price']}$. Sector: {c['sector']}. Score 0-100, Sigma 0-50 JSON."
            async with session.post("https://openrouter.ai/api/v1/chat/completions",
                ...
                if resp.status == 200:
                    ...
                    return float(content.get('score', 80)), float(content.get('sigma', 15))
        except: pass
        return 80.0, 15.0 

    async def run_cycle(self):
        """Cycle de trading avec double Kill-Switch (Daily & Total)."""
        ...
        # 1. Kill-Switch Journalier (2%)
        daily_dd = (equity - float(acc.last_equity)) / float(acc.last_equity) if float(acc.last_equity) > 0 else 0
        if daily_dd <= -GOUVERNANCE["MAX_DAILY_DRAWDOWN_PCT"]:
            logging.critical("🚨 Kill-Switch Journalier activé. Liquidation.")
            self.alpaca.close_all_positions()
            SYSTEM_STATE["status"] = "CRITICAL_HALT"; return

        # 2. Kill-Switch Total (10%) - Audit 5.5
        total_dd = (equity - self.initial_equity) / self.initial_equity
        if total_dd <= -GOUVERNANCE["MAX_TOTAL_DRAWDOWN_PCT"]:
            logging.critical("🚨 Kill-Switch Total activé. Arrêt définitif.")
            self.alpaca.close_all_positions()
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for c in candidates:
                ...
                # Check Exposition Sectorielle (avec cache cohérent)
                ...
                score, sigma = await self.get_ai_score(session, c)
                ...
                if mode:
                    ...
                    if qty > 0:
                        try:
                            ...
                            with sqlite3.connect(DB_PATH) as conn:
                                conn.execute("INSERT INTO trades (client_id, symbol, qty, entry_price, status, mode, consensus, dispersion, sector) VALUES (?,?,?,?,?,?,?,?,?)", 
                                             (uuid.uuid4().hex, c['symbol'], qty, c['price'], 'OPEN', mode, score, sigma, c['sector']))
                            logging.info(f"✅ APEX EXECUTION : {c['symbol']}")
                        except: pass
...
async def main():
    ...
    engine = TitanEngine()
    logging.info(f"🛡️ Titan v5.6 Apex-Guardian Online (Base Stable)")
    while True:
