...
# SÉCURITÉ : Récupération du token via le service systemd
...
class TitanEngine:
    ...
    async def get_ai_score(self, session, sym, sector, price):
        ...
        if not OR_KEY:
            return 82.0, 12.0, "IA_DISABLED: Clé OpenRouter manquante"

        prompt = (
            f"Analyse quantitative PEAD pour {sym} (Secteur: {sector}). "
            f"Prix actuel: {price}$. "
            f"Évalue le potentiel de drift à 3 jours après les résultats. "
            f"Réponds uniquement au format JSON : {{\"score\": 0-100, \"sigma\": 0-50, \"reason\": \"[FUND]...|[TECH]...|[VETO]...\"}}"
        )
        ...
        for i in range(5):
            try:
                async with session.post("https://openrouter.ai/api/v1/chat/completions", 
                                        headers=headers, json=payload, timeout=15) as resp:
                    if resp.status == 200:
                        res = await resp.json()
                ...
                if i == 4: SLOG.log("ai_api_error", level="error", error=str(e))
                await asyncio.sleep(2**i)
        
        return 80.0, 15.0, "FALLBACK: Erreur de communication Grok"
                ...
                for _, row in candidates.iterrows():
                    ...
                    if mode and SYSTEM_STATE["allocation"][mode] > 0:
                        ...
                        if qty > 0:
                            ...
                            self.alpaca.submit_order(**params)
                            with sqlite3.connect(DB_PATH) as conn:
